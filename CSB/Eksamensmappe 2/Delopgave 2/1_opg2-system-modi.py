...
def euler(f, t_start, x0, t_stop, n):
    h = (t_stop - t_start)/n            # Skridtlængde        
    t = [t_start]
    x = [x0]                            # Startværdier
    tn = t_start
    xn = x0
    ...
    for k in range(n):
        tn = tn + h                     # 
        xn = xn + h*f(tn,xn)            # Eulerskridt     
        t.append(tn)                    # Tilføj
        x.append(xn)
        
    return np.array(t), np.array(x)     # Returne
...
def RK4(f, t_start, x0, t_stop, n):
    ...
    for k in range(n):
        ...
        k4 = f(tn + h,xn + h*k3)
        tn = tn + h
        xn = xn + h*(k1 + 2*(k2 + k3) + k4)/6   # RK4
        t.append(tn)                            # Tilføjer
        x.append(xn)
