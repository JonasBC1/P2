...
\noindent
% 
Hvis $f(\mathbf{u}) \leq f(\mathbf{v})$ for $\textbf{v}$ i nærheden af $\mathbf{u}$, siges $\mathbf{u}$ at være et \textit{lokalt minimum} af $f$, og $\mathbf{u}$ siges at være et \textit{globalt minimum}, hvis det gælder  $f(\mathbf{u}) \leq f(\mathbf{v})$ for alle $\mathbf{v} \in \R^n$.
