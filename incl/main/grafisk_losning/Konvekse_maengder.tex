...
\end{defn}
%
På figur \ref{fig:neej2} ses to vektorer $\mathbf{x}$ og $\mathbf{y}$ i $\R^n$ og den konvekse kombination $\frac{1}{2} \mathbf{x}+\frac{1}{2} \mathbf{y}$, hvor $\lambda_x = \lambda_y = \frac{1}{2}$, og den konvekse hylster af vektorerne er en ret linje, der er markeret med blå. 
\\\\
%
\input{fig/tikz/geometri/konveks_eks_r2}
%
På figur \ref{fig:neeej} ses derimod tre vektorer $\mathbf{x}$, $\mathbf{y}$ og $\mathbf{z}$ i $\R^n$, og den konvekse hylster af vektorerne er en afgrænset flade, der på figuren er markeret med blå.
%
...
Bemærk, at en polyhedron er fællesmængden af et endeligt antal halvrum, og at (b) dermed er bevist jævnfør (a). 
	\item  Dette punkt bevises ved induktions bevis.
...
Lad $\lambda_1,\ldots,\lambda_{k+1}$ være ikke negative skalarer som hvis sum er $1$. 
...
Koefficienterne $\lambda_i/(1-\lambda_{k+1})$, for $i=1,\ldots,k$, er ikke negative og summerer til 1. Ved induktionsantagelsen gælder,  $\sum^{k}_{i=1}\lambda_i\textbf{x}_i/(1-\lambda_{k+1})\in S$. Da $S$ er konveks fåes det at $
\sum^{k+1}_{i=1}\lambda_i\textbf{x}_i\in S$. 
%	\input{incl/main/grafisk_losning/konveks_bevis_c}
