\subsection{Span}
% 1.6
\textbf{Spannet} er den del af vektorrummet, som en given mængde vektorer dækker over. 
%
\begin{defn}{}{}
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
Egenskaberne vedrørende span kan udtrykkes på forskellige måder, hvilket leder til følgende sætning:
% 
\begin{thm}{}{}
%
...
\begin{proof}
%
Udsagn $(a)$ og $(b)$ er ækvivalente, eftersom det er en forudsætning, at der kan skabes en linear kombination af $A\mathbf{x} =\mathbf{b}$ for ethvert $\mathbf{b}$ i $\R^m$, da det netop er definitionen af spannet, der dækker $\R^m$. Dette er jævnfør sætning \ref{thm:konsistens}.
%
%
%%%%%%%%% Spannen? Spannet? Vælg én. (jeg stemmer spannet). 
%
%
...
Det følger derfor af sætning \ref{thm:konsistens}, at den sidste række ikke kan være en nulrække, da den på grund af standardvektoren $\mathbf{e}_m$ således vil have pivotindgang i sidste søjle og derfor vil ligningssystemet være inkonsistent.
Dermed er rangen af $rang(A)=m$, hvilket beviser (c).
...
%%% Til ovenstående: At der ikke findes en ikke-nulrække i $B$
...
Hvis en vektor $\mathbf{w}$ er i $\text{span}\{ \mathbf{u}_1, \mathbf{u}_2 , \ldots , \mathbf{u}_k , \mathbf{v}\}$, kan denne opskrives $\mathbf{v}=c_1\mathbf{u}_1+c_2\mathbf{u}_2+ \ldots + c_k\mathbf{u}_k+b\mathbf{v}$, hvor $c_1,c_2,\ldots, c_k$ er skalarer.
